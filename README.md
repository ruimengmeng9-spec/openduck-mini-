...
- V14 加倍进度奖励并放宽姿态约束，仅恢复到 `-0.0065 m/s`，仍未解决；
- **V15 定位并修复了真正的根因**：①参考动作网格步长为 ±0.074，`-0.03` 这类低速后退指令
  会**精确落到 dx=0.000 的站立参考**上，配合 `imitation=0.25` 把策略锚死在站立；
  ②超速惩罚的允许带宽（0.049 m/s）比步态速度波动还窄，使「移动」本身被罚。
  修复后后退速度从 −0.004 提升到 **−0.072 ~ −0.112 m/s**（超过历史最好的 V12 的 −0.097）；
- **稳定性仍未解决**。开环回放证明数据集里的后退参考步态 `−0.074` 是**鲁棒稳定**的
  （5 个种子 × 5 档扰动全部不跌倒），但 V12–V19 全部在 **1.6–2.8 秒后跌倒**；
  模仿权重从 0 扫到 20、课程对准网格、去掉零指令混合、提高姿态惩罚均无效。
  瓶颈是 PPO 学不会跟踪该参考，而非激励方向问题。详见
  [TRAINING_NOTES.md](TRAINING_NOTES.md) 与
  [results/backward_v12_to_v19_20260918.json](results/backward_v12_to_v19_20260918.json)；
- 爬起（getup）专项已打通物理前提（躯干碰撞体，原模型不受影响），但**10 轮训练
  均未学会起身**，包括两路 **1 亿步**训练（仍 0/12）。诊断结论：奖励设计无误
  （站立 +2.34/步 vs 躺平 −3.34/步），但随机探索从完全倒地出发最高只到 `up_z≈0.87`，
  站立奖励在概率上不可达；已验证的行走策略在新旧场景中表现完全一致，物理未被破坏。
  纯稀疏奖励需要参考动作或脚本化起身的模仿信号来提供可行中间路径。详见
