...
- **V15 定位并修复了真正的根因**：①参考动作网格步长为 ±0.074，`-0.03` 这类低速后退指令
  ...
  修复后退速度恢复到 **−0.072 / −0.095 m/s**（V12 水平），但**仍约 2 秒后跌倒**；
  V16 单纯提高姿态惩罚无效（−0.066 / −0.090，仍跌倒）；
  V17 把课程对准网格值 `-0.074` 并恢复模仿权重以补步态先验。算术核对见
  `diagnostics/diag_backward_reward.py`，实测对比见
  [results/backward_v15_20260918.json](results/backward_v15_20260918.json)；
