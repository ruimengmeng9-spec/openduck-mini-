"""Collect simulated teacher actions for a narrow -0.15 rad/s turn command.
...
def main() -> None:
    ...
    parser.add_argument("--student-band", choices=("narrow", "wide"), default="narrow")
    ...
    if not 0.12 <= args.negative_speed_min <= args.negative_speed_max <= 0.18:
        parser.error("negative speeds must satisfy 0.12 <= min <= max <= 0.18")
    ...
    for seed in range(args.seed_start, args.seed_start + args.seeds):
        simulation = DuckSimulation(REPO, OFFICIAL, output_root=args.output.parent, warmup_s=3.0)
        sim = simulation.sim
